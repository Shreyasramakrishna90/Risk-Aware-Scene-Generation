...
def preprocessing(path,folders):
    for folder in folders:
        root = path + folder + "/"
        dirlist = [item for item in os.listdir(root) if os.path.isdir(os.path.join(root,item))]
    # for index,dir in enumerate(dirlist):
    #     os.rename(root + dir, root + "scene%d"%index)

    return natsorted(dirlist),root

def split_data(dirlist,root,paths):
    numImages = 0
    inputs = []
    for dir in dirlist:
        if dir not in ["calib","train"]:
            path = root + dir + "/" + "rgb_detector" + "/"
            print(path)
            numFiles = len(glob.glob1(path,'*.png'))
            print(numFiles)
            numImages += numFiles
    ...
    for i in range(0,len(inputs)):
        if i % 5 == 0:
            cv2.imwrite(paths[1] + "/frame%d.png"%k,inputs[i])
            k+=1
        else:
            cv2.imwrite(paths[0] + "/frame%d.png"%j,inputs[i])
            j+=1
            #comp_inp.append(calib_path+ "/",inputs[i])
    print("Total number of train images:%d" %j)
    print("Total number of calib images:%d" %k)
    # return inputs, comp_inp
    return inputs

def create_folders(path,train_folder):
    paths = []
    folders = ["train","calib"]
    for folder in folders:
        data_folder = path + train_folder[0] + "/" + folder
        os.makedirs(data_folder, exist_ok=True) #creates a new dir everytime with max number
...
if __name__ == '__main__':
    path = "/isis/Carla/autopilot_Carla_ad/leaderboard/"
    folders = ["training_data"]
    paths = create_folders(path,folders)
    dirlist,root = preprocessing(path,folders)
    print(dirlist)
    inputs = split_data(dirlist,root,paths)
