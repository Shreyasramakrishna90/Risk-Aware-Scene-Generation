    ...
    def on_carla_tick():
        ...
        for actor in CarlaDataProvider._actor_location_map:
            if actor is not None and actor.is_alive:
                ...
                #print(CarlaDataProvider._actor_location_map[actor])
                #print(spawn_points[84].location)
                distance = CarlaDataProvider._actor_location_map[actor].distance(spawn_points[84].location)
                #print(distance)
...
class CarlaActorPool(object):
    ...
    def create_blueprint(model, rolename='scenario', hero=False, autopilot=False, color=None, actor_category="car"):
        ...
        if color:
            if not blueprint.has_attribute('color'):
                print("WARNING: Cannot set Color ({}) for actor {} due to missing blueprint attribute".format(
                        color, blueprint.id))
            else:
                default_color_rgba = blueprint.get_attribute('color').as_color()
                print(default_color_rgba)
    ...
    def setup_actor(model, spawn_point, rolename='scenario', hero=False, autopilot=False,
        ...
        if random_location:
            actor = None
            while not actor:
                spawn_point = CarlaActorPool._spawn_points[85] #random.choice(CarlaActorPool._spawn_points)
