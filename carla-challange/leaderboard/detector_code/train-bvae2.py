...
def load_images(path,trainingFolders):
    numImages = 0
    inputs = []
    paths = path + trainingFolders[0] + "/"
    print(paths)
...
def load_data(path,trainingFolders):
    #Loading images from the datasets
    csv_input = load_images(path,trainingFolders)
...
def CreateModels(n_latent=40, sample_enc=sample_func, beta=1.0, C=0):
    model = Sequential()
...
def train(X,autoencoder,data):
    ...
    autoencoder.fit(X_train, X_train,epochs=120,batch_size=16,shuffle=True,validation_data=(X_test, X_test),callbacks=callbacks_list, verbose=2)
...
if __name__ == '__main__':
    # path to USB
    path = "/isis/Carla/autopilot_Carla_ad/leaderboard/training_data/"
    # list of folders used in training
    trainingFolders = ["train"]#["clear_noon1","clear_noon2","clear_noon3","clear_noon4","clear_noon5"]
    #Only parameters that has to be changed
    Working_directory = "/isis/Carla/carla-dockers-trial/carla-challange/leaderboard/detector_code/new_trial/"#working directory
    Working_folder = 'center-B-1.0-40'#experiment
    ...
    if not os.path.exists(Working_path):
        os.makedirs(Working_path, exist_ok=True) #creates a new dir everytime with max number
    print("loading image")
    inp = load_data(path,trainingFolders)
    print("created model")
    autoencoder,encoder,decoder,z_log_var = CreateModels()# Running the autoencoder model
    print("training model")
    train(inp,autoencoder,data)#Train the model with the data
    print("testing model")
